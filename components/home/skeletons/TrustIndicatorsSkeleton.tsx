      ...
      <div className="container mx-auto px-4 ">
        <div className="flex justify-between items-center gap-4 text-left">
          <div className="flex items-center gap-2">
            <Skeleton className="min-h-0.5w-8 h-8 rounded-full" />
            <div className="flex items-center gap-1">
              <Skeleton className="h-5 min-w-8" />
              <div className="flex">
                {[1, 2, 3, 4, 5].map((i) => (
                  <Star key={i} className="min-w-4 h-4 text-gray-300 fill-current" />
                ))}
              </div>
            </div>
          </div>
          <div className="flex-1 relative overflow-hidden">
            ...
            <Skeleton className="p-1 rounded-full min-w-6 h-6" />
            
            <div className="flex gap-4 overflow-x-auto scrollbar-hide scroll-smooth mx-8 px-8 w-full">
              {[...Array(4)].map((_, index) => (
                <Skeleton key={index} className="h-5 min-w-5 w-24" />
              ))}
            </div>
